...
export default function ProjectCards(props) {
  ...
  function usePerspective() {
    ...
    rotation[0] = -(pageY - cardOffSetY - height / 2) / constrain; // rY
    ...
    let transforms = {
      frame: `rotateX(${rotation[0]}deg) rotateY(${rotation[1]}deg)`,
      background: `translateY(${translation[0]}px) translateX(${translation[1]}px)`,
    };
    ...
    <GridContainer>
      {projects.map((project) =>
        props.filter === 'All' || project.tags.some(applyFilter) ? (
          <div>
            <CardWrapper
              key={project[localLanguage.toString()].title}
              onMouseMove={(event) => {
            ...
            >
              <IconContainer
                style={{ transform: styles.frame + 'translateZ(30px)' }}
              >
